...
namespace WonderGather.Tests
{
    public sealed class FactionPersistenceTests
    {
        ...
        [Test] public void UnsupportedIncompleteAndUnknownFilesAreLeftIntact()
        {
            ...
            foreach(var invalid in new[]{valid.Replace("\"version\": 1","\"version\": 99"),valid.Replace("\"workers\": 8,",""),valid.Replace("{","{\"futureData\":42,"),"{ incomplete",valid.Replace("\"workers\": 8","\"workers\": -2")})
            {
        ...
        [Test] public void ActiveRenameAndConcurrentWriteConflictsPreserveExternalEdits()
        {
            ...
            var entry=store.Read(id);entry.Record.Workers=2;store.Save(entry.Record,entry.Token);
